...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  /** When true, thinking.delta must NOT overwrite the status bar */
  ...
  /**
   * Fallback status timer for thinking phases.
   *
   * IMPORTANT: This must NEVER write raw thinking/chain-of-thought text to
   * the status bar. The bridge's explicit status.update events ('Thinking…',
   * 'Generating…', etc.) are the authoritative source of status labels.
   * This fallback only fires when the bridge has NOT emitted a status.update,
   * and even then it uses a clean generic label — never raw model output.
   */
  const scheduleThinkingStatus = () => {
    // If the status was explicitly set by a status.update event from
    // the bridge ('Thinking…', 'Generating…', 'Running Bash…'), don't
    // overwrite it with a fallback.
    if (statusExplicitlySet) return

    pendingThinkingStatus = statusFromBusy()
    ...
    thinkingStatusTimer = setTimeout(() => {
      thinkingStatusTimer = null
      patchUiState({ status: pendingThinkingStatus || 'Thinking…' })
    }, STREAM_BATCH_MS)
  }
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // forging a brand-new one.  Mirrors classic CLI's `coder -c` /
    // `coder --tui` muscle memory and addresses the audit's "session
    ...
    switch (ev.type) {
      ...
      case 'thinking.delta': {
        ...
        if (text !== undefined) {
          ...
          // Fallback: if the bridge has not yet emitted a status.update
          // (e.g. thinking/delta arrived before thinking/start from the
          // provider), schedule a clean generic label.  Never feed raw
          // thinking/chain-of-thought text to the status bar — it is
          // only for the reasoning panel (<thinking> drawer).
          scheduleThinkingStatus()
      ...
      case 'tool.complete': {
        ...
        // When a tool fails, surface the error in the status bar so the user
        // can see what went wrong without scrolling through the transcript.
        // After a brief display, restore the normal status.
        if (ev.payload.error) {
          const toolName = ev.payload.name ?? 'tool'
          const brief = ev.payload.error.length > 60
            ? `${ev.payload.error.slice(0, 57)}…`
            : ev.payload.error
          setStatus(`Tool failed - ${toolName}: ${brief}`)
          restoreStatusAfter(5000)
        }
      ...
      case 'error':
        // Do NOT call turnController.recordError() here — it immediately
        // calls idle() which sets busy=false and clears all turn state
        // (pendingSegmentTools, segmentMessages, turnTools) BEFORE the
        // error message is visible to the user.  Instead we defer the
        // state cleanup until after the error has been displayed.
        {
          ...
          // Show the error status prominently, then clean up turn state
          // and return to ready after the user has had time to read it.
          setStatus('error')
          turnController.idle()
          restoreStatusAfter(4000)
        }
      ...
      default:
        // Exhaustiveness check: if a new GatewayEvent type is added
        // to the union, TypeScript will error here — forcing us to
        // handle it explicitly rather than silently dropping events.
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        ; ((_exhaustive: never) => {})(ev)
    }
  }
}
