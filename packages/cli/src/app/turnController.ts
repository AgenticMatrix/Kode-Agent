...
import type { SessionInterruptResponse, SubagentEventPayload } from '../gateway/types.js'
...
class TurnController {
  bufRef = ''
  /** Names of tools in the current turn batch that completed with an error. */
  ...
  recordToolComplete(
    ...
    if (this.interrupted) {
      // Even when interrupted, record the tool error so the UI can show
      // what went wrong.  Silently returning here would drop the error
      // and the user would never know a tool failed.
      if (error) {
        this.failedToolNames.add(fallbackName ?? 'tool')
        this.pushActivity(`Tool failed: ${error}`, 'error')
      }
      return
    }
  ...
  private completeTool(
    ...
    // Track error state for the turn trail.  When the last tool in the
    // batch finishes we use this flag to pick a status message that tells
    // the user whether something went wrong (instead of always showing
    // the misleading "analyzing tool output…").
    if (error) {
      this.failedToolNames.add(name)
    }

    const next = this.turnTools.filter(item => !sameToolTrailGroup(label, item))

    if (!this.activeTools.length) {
      if (this.failedToolNames.size > 0) {
        const failed = [...this.failedToolNames].join(', ')
        next.push(`Tool "${failed}" failed`)
        this.failedToolNames.clear()
      } else {
