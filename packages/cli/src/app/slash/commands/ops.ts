...
export const opsCommands: SlashCommand[] = [
  ...
  {
    help: 'interactive diff viewer — open split/unified diff overlay',
    name: 'diff',
    run: (_arg, ctx) => {
      // Open the diff viewer overlay.  When invoked without arguments, show
      // a placeholder; the user can use u/s to toggle unified/split mode.
      // Future: /diff rollback <hash> to populate from a checkpoint.
      patchOverlayState({
        diffView: {
          oldContent: `// Old version — paste content or use /rollback diff\n`,
