...
export class DeepSeekProvider extends OpenAICompatProvider {
  ...
  // -----------------------------------------------------------------------
  // Extension Points — DeepSeek-specific headers, pricing, models
  // -----------------------------------------------------------------------
  ...
  protected override buildRequestHeaders(): Record<string, string> {
    return {
      ...
      // Enable DeepSeek prefix caching for cost optimization
  ...
  override async listModels(): Promise<ModelInfo[]> {
    return [
      ...
      },
      {
        ...
        contextWindow: 64_000,
        maxOutputTokens: 8192,
        supportsTools: false, // Reasoner has limited tool support
        supportsVision: false,
