...
import { getStatusVerb, VERBS } from '../content/verbs.js'
...
// jitter when the status text changes between short/long labels (e.g.
// "Thinking" → "Generating" → "Running Bash").
...
interface IndicatorRender {
  frame: string
  intervalMs: number
  // When false, FaceTicker hides the status verb and just shows the
  // glyph + duration.  Lets `unicode` stay minimal while the other
  // styles keep the status-label flavour users associate with the
  // running… status.
  showVerb: boolean
}
...
const renderIndicator = (style: IndicatorStyle, tick: number): IndicatorRender => {
  ...
  // the cleanest possible status, so no status verb either.
...
function FaceTicker({ color, startedAt }: { color: string; startedAt?: null | number }) {
  ...
  // Use the deterministic getStatusVerb() to show actual LLM state from the
  // bridge's status.update events ('Thinking', 'Generating', 'Running Bash')
  // instead of rotating through random verbs. Falls back to 'Thinking' when
  // the current status is generic / transitional.
  const verb = getStatusVerb({ busy: ui.busy, status: ui.status })
  ...
  return (
    <Box height={1}>
      <Box flexDirection="row" flexShrink={1} overflow="hidden" width={leftWidth}>
        ...
        {modeLabel ? onModeLabelClick ? (
          <Box flexShrink={0} onClick={(e: { stopImmediatePropagation?: () => void }) => { e.stopImmediatePropagation?.(); onModeLabelClick(); }}>
...
interface StatusRuleProps {
  ...
  /** Agent mode label: "COORDINATOR", "WORKER", or undefined for default */
  modeLabel?: string
  /** Callback when mode label is clicked (opens coordinator dashboard) */
  onModeLabelClick?: () => void
