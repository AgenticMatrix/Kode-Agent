...
import type { InputEvent, Key } from '@coder/tui'
import * as Ink from '@coder/tui'
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.CODER_TUI_TERMUX_FAST_ECHO ?? '').trim().toLowerCase()
...
/**
 * When true, TextInput passes arrow keys and Enter through to the parent
 * `useInput` handler instead of consuming them internally.  Set by the
 * slash-command popup so the user can navigate the command list with
 * ↑↓ and confirm with Enter while the underlying composer remains live
 * for typing the filter prefix.
 */
let slashPopupActive = false

/** Call with `true` when the slash-command popup opens, `false` when it closes. */
export function setTextInputSlashPopupActive(active: boolean): void {
  slashPopupActive = active
}
...
export const shouldPassThroughToGlobalHandler = (
  ...
  key.pageDown ||
  key.escape ||
  (slashPopupActive && (key.upArrow || key.downArrow || key.return)) ||
