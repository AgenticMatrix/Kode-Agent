...
function validateCronField(value, field) {
    ...
    if (value.startsWith('*/')) {
        ...
        if (isNaN(step) || step < 1)
            return `Invalid step in ${field}: ${value}`;
        return null;
    }
    // Check lists
 ...
 */
function computeNextRun(cron, from) {
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        if (matchesField(minuteField, minute) &&
...
export function getScheduledTasksPath() {
