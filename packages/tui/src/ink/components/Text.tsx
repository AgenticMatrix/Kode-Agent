...
type BaseProps = {
  ...
  /**
   * Shortcut for dim + color — applies both dim and sets color in one prop.
   * Resolved at render time: dimColor=#xxx is equivalent to dim color=#xxx.
   */
  readonly dimColor?: boolean | Color
...
export function shouldUseAnsiDim(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CODER_TUI_DIM ?? '').trim()
