...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CODER_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Coder on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CODER_TUI_TRUECOLOR=1.
