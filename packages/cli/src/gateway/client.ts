...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.CODER_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.CODER_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
const resolveGatewayAttachUrl = () => {
  const raw = process.env.CODER_TUI_GATEWAY_URL?.trim()

  return raw ? raw : null
}

const resolveSidecarUrl = () => {
  const raw = process.env.CODER_TUI_SIDECAR_URL?.trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = process.env.CODER_PYTHON?.trim() || process.env.PYTHON?.trim()
...
const asWireText = (raw: unknown): string | null => {
  ...
  if (ArrayBuffer.isView(raw)) {
    ...
    return _wireDecoder.decode(raw as any)
  }
...
export interface IGatewayClient {
  ...
  on(event: 'event', handler: (ev: GatewayEvent) => void): this
  on(event: 'exit', handler: (code: number | null) => void): this
  off(event: 'event', handler: (ev: GatewayEvent) => void): this
  off(event: 'exit', handler: (code: number | null) => void): this
  emit(event: 'event', ev: GatewayEvent): boolean
...
export class GatewayClient extends EventEmitter implements IGatewayClient {
  ...
  private startSpawnedGateway(root: string) {
    ...
    const cwd = process.env.CODER_CWD || root
  ...
  start() {
    const root = process.env.CODER_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
