  ...
  const nodes = useMemo(() => {
    ...
    while (i < lines.length) {
      ...
      if (fence) {
        ...
        // Diff file header: extract `+++ b/path` from the block
        const diffFileName = isDiff
          ? (() => {
          ...
          <Box flexDirection="column" key={key} paddingLeft={2}>
            ...
            {diffFileName && (
              <Box flexDirection="row" marginBottom={1}>
                ...
                <Text color={t.color.muted}>— {diffFileName}</Text>
              </Box>
            )}
            ...
            {block.map((l, j) => {
              ...
              const displayLine = isDiff && l.length > MAX_DIFF_LINE
                ? l.slice(0, MAX_DIFF_LINE) + '…'
                : l
              ...
              return (
                <Text
                  ...
                  color={
                    hunk ? t.color.accent
                      : add ? t.color.diffAddedWord
                      : del ? t.color.diffRemovedWord
                      : undefined
                  }
