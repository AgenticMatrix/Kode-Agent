...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.KODE_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Kode on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via KODE_TUI_TRUECOLOR=1.
