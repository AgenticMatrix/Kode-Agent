import { useApp, useHasSelection, useSelection, useStdout, useTerminalTitle } from '@coder/tui'
...
export function useMainApp(gw: IGatewayClient) {
  const { exit } = useApp()
  ...
  const { pagerPageSize } = useInputHandlers({
    ...
    gateway,
    terminal: { hasSelection, selection, stdout },
    voice: {
  ...
  const answerApproval = useCallback(
    (choice: string) =>
      respondWith(
        'approval.respond',
        {
          choice,
          session_id: ui.sid,
          request_id: overlay.approval?.request_id ?? overlay.approval?.tool_use_id,
        },
        () => {
    ...
    [respondWith, ui.sid, overlay.approval?.request_id, overlay.approval?.tool_use_id]
  )
