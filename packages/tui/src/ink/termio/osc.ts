...
export function shouldEmitClipboardSequence(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (
    env.CODER_TUI_FORCE_OSC52 ??
    env.CODER_TUI_CLIPBOARD_OSC52 ??
    env.CODER_TUI_COPY_OSC52 ??
    ''
  ).trim()
 ...
 *     CODER_TUI_FORCE_OSC52=0, no OSC 52 sequence will be written. If
...
export function shouldUseNativeClipboard(
  ...
  // apply. Even when CODER_TUI_FORCE_OSC52=1 forces a tmux-passthrough
 ...
 * DCS and prefix+] still works. See related discussion on tmux clipboard behavior.
 *
...
export async function setClipboard(text: string): Promise<ClipboardResult> {
  ...
  // (see related discussion on clipboard race conditions).
  ...
  // CODER_TUI_FORCE_OSC52=1 we DO additionally emit a tmux-passthrough
  ...
  // CODER_TUI_FORCE_OSC52=0 (otherwise the clipboard write becomes a
...
/** Emit OSC 21337 tab-status sequences. Terminals that do not recognize them discard silently. */
export function supportsTabStatus(): boolean { return true }
